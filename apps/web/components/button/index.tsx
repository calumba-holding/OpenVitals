  ...
  "cursor-pointer inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-accent-500 focus-visible:ring-offset-2",
  {
    variants: {
      variant: {
        ...
        outline:
          'border border-neutral-200 bg-white shadow-xs hover:bg-neutral-50 text-neutral-700',
        secondary:
          'bg-neutral-100 text-neutral-700 shadow-xs hover:bg-neutral-200',
        ghost:
...
function Button({
  ...
  const button = (
    ...
    >
      ...
      {shortcut && (
        <kbd className="shrink-0 hidden rounded border border-neutral-200 bg-neutral-100 px-1.5 py-0.5 text-[10px] font-light text-neutral-400 md:inline-block">
          {shortcut}
        </kbd>
      )}
      {right}
  ...
  if (tooltip) {
    return (
      <Tooltip
        content={tooltip}
        delayDuration={200}
        contentClassName="px-2 py-1 rounded-sm"
      >
        {button}
      </Tooltip>
    );
