...
export function AnimatedEmptyState({
  ...
  cardContent = ({ icon: Icon }: { index: number; icon: IconType }) => (
    <>
      ...
      <div className="h-2.5 w-28 min-w-0 rounded-sm bg-neutral-100" />
    </>
  ),
  ...
  return (
    <div
      className={cn(
        'flex flex-col items-center justify-center gap-6 rounded-xl border border-neutral-200 bg-white px-4 py-10 md:min-h-[420px]',
        className,
      )}
    >
      ...
      <div className="max-w-xs text-pretty text-center">
        <span className="text-lg font-medium text-neutral-900 font-body">{title}</span>
        {!!description && (
          <p className="mt-2 text-pretty text-sm text-neutral-500 font-body">{description}</p>
        )}
      </div>
      <div className="flex items-center gap-2">
        ...
        {learnMoreHref && (
          <Link
            href={learnMoreHref}
            className={cn(
              'rounded-lg border border-neutral-200 bg-white px-4 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-50 transition-colors',
              learnMoreClassName,
            )}
          >
            Learn more
